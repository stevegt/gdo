...
'''
Figure out whether we're I/O or CPU bound.
...
def hash_stream(srcfh, seed, word_size, double=False):
	assert seed != 0
	numpy.random.seed(seed)
	if double: streamhash = hashlib.sha256()
	cdc = CDC(lambda: srcfh.read(), seed, word_size)
	totalsize = 0
	while True:
		size = cdc.get_chunk()
		if size == 0:
			break
		totalsize += size
		if double: streamhash.update(cdc.chunk)
		chunkhash = hashlib.sha256(cdc.chunk)
		yield size,"sha256:%s" % chunkhash.hexdigest()
	if double: 
		yield None,None
		yield totalsize,"sha256:%s" % streamhash.hexdigest()
